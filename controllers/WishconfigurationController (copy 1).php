...
class WishconfigurationController extends WishmainController
{
    ...
    public function actionIndex()
    {
        ...
        if ($postData = Yii::$app->request->post())
        {
            ...
            $wish_url=trim($_POST['redirect_url']);

            $this->getToken($client_id,$client_secret_key,$wish_code);
          ...
          /*  $sale_price=$_POST['sale_price'];
           ...
           // $productTax_code=$_POST['tax_code'];
            ...
            if (!empty($isConfigurationExist)){
                Data::sqlRecords("UPDATE `wish_configuration` SET `client_id`='".$client_id."',`client_secret_key`='".$client_secret_key."',`code`='".$wish_code."', `access_token`='' where `merchant_id`='".MERCHANT_ID."'", null, "update");
            } else{                
                //save api credentials
                Data::sqlRecords("INSERT INTO `wish_configuration` (`merchant_id`, `client_id`,`client_secret_key`,`code`,`access_token`,`redirect_url`) values(".MERCHANT_ID.",'".$client_id."','".$client_secret_key."','".$wish_code."') ", null, "insert");
            }
    ...
    public function getToken($client_id,$client_secret,$code){
       ...
       // https://merchant.wish.com/api/v2/oauth/access_token?client_id=59957f5835e73b36f9a308da&client_secret=a21fa03d71c1426b9d33c76892f0ffd7&code=f8552350ab5f4f909d77756819ce2564&grant_type=authorization_code&redirect_uri=https://www.cedcommerce.com
        ...
        if (is_array($request)) {
            ...
            if($request['code']==0){
               $data = $request['data'];
               Mage::getModel('core/config')->saveConfig('wish_options/ced_wish/wish_token', $data['access_token']);
               Mage::getModel('core/config')->saveConfig('wish_options/ced_wish/wish_refresh_token', $data['refresh_token']);
               return array('success'=>true ,$data['access_token']);
            }
