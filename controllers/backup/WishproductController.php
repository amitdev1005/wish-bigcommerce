...
class WishproductController extends WishmainController
{
    ...
    public function actionEditdata()
	{
	    	...
	    	$model = WishProduct::find()->joinWith('jet_product')->where(['wish_product.id'=>$id])->one();

	    	$Category=[];
	    	$category_path="";
	    	// $query="SELECT category_name,category_id, parent_id, attributes, attribute_values, walmart_attributes, walmart_attribute_values FROM `wish_category_map` WHERE category_id='".$model->category."' LIMIT 1";
	    	...
	    	if(is_array($Category) && count($Category)>0)
	    	{
                ...
                // $category_id = WishCategory::getCatoryName($Category['category_id']);
	    		...
	    		// {
                    ...
                    // $category_temp= preg_split('#(?<=\d)(?=[a-z])#i',$category_path);
                    // $parent_id = $category_path = $category_temp['1'];
                    $temp_cname = str_replace($category__name,'', $category_path);
                    $category_path = $temp_cname.'->'.$category__name;
                    // print_r($temp_cname);die();
                    // preg_match('/$category__name/', 'foobarbaz', $matches, PREG_OFFSET_CAPTURE);
	    		// }
	    	}
            // print_r($category_path);die();
	    	$attributes=[];
	    	//$optional_attr=[];
	    	$requiredAttrValues=[];
	    	$optionalAttrValues=[];
...
//         }
    ...
    public function actionAjaxBulkUpload()
    {
        ...
        {
            $data  = $this->wishConfig->updateBulkPromotionalPriceOnWalmart($selection);
            if(isset($data['success']))
            {
                Yii::$app->session->setFlash('success', "Successfully Updated Promo Price on Wish. FeedId : ".$data['feedId']);
            }
        ...
        else 
        {
            ...
            try {
                ...
                if(isset($productResponse['errors'])){
                    ...
                    $returnerrorArr = ['error'=>true, 'message'=>$productResponse['errors']];
                    //print_r($returnerrorArr);die;
                    
                }
                if(isset($productResponse['success']))
                {
                
                    /*$feed_file = isset($productResponse['id']);
                    $query="INSERT INTO `wish_product_feed`(`merchant_id`,`feed_id`,`product_ids`,`feed_file`)VALUES('".MERCHANT_ID."','".$productResponse['id']."','".$ids."','".$feed_file."')";
                    Data::sqlRecords($query,null,"insert");
                    //echo $query;die;

                    $msg = "product feed successfully submitted on wish.";
                    $feed_count = count($productResponse['id']);
                    $feedId = $productResponse['id'];
*/
                    $msg = "product feed successfully submitted on wish.";
                    $returnArr = ['success'=>true, 'message'=>$msg /*'count'=>$feed_count, 'id'=>$feedId*/];
                    return json_encode($returnArr);
                }
                elseif(isset($productResponse['errors'])) {
                ...
                if (count($returnerrorArr['error'])>0) {
                	...
                	foreach ($productResponse['errors'] as $error) {
                      ...
                      /*  $skus="SELECT `sku` FROM jet_product where `bigproduct_id`='". $selectedProducts."'" and `merchant_id`=MERCHANT_ID;
                        $sku = Data::sqlRecords($skus,"one","select");
                        print_r($sku);die("dgfg");
                       if($productsku == $model2){
                        die("fdfgdf");
                       }*/
                        else{
    ...
    public function actionPricepost()
    {
        ...
        if($count) {
            $response = $this->wishConfig->updatePriceOnWish($selectedProducts, "product");
            if(isset($response['errors']))
    			...
    			foreach ($skus as $sku) {
    				...
    				if (isset($feed_data['ItemRetireResponse'])){
    					...
    					if (isset($feed_data['errors']['error']['code']) && $feed_data['errors']['error']['code'] == "CONTENT_NOT_FOUND.GMP_ITEM_INGESTOR_API" && $feed_data['errors']['error']['field'] == "sku") {
    						$errors[] = $sku['option_sku'] . ' : Product not Uploaded on Wish.';
    					} else {
   ...
   /* public function actionBatchretire()
    ...
    {
        $query = 'select count(*) as products from (SELECT * FROM `wish_product` WHERE `merchant_id`= "' . MERCHANT_ID . '") as wal INNER JOIN (SELECT * FROM `jet_product` WHERE `merchant_id`= "' . MERCHANT_ID . '") as jet ON jet.bigproduct_id=wal.product_id where wal.merchant_id="' . MERCHANT_ID . '" LIMIT 0,1';
    ...
    */
    ...
    public function actionProductstatus()
    {
        ...
        try {
            ...
            $offset = $index * $getItemsCount;
            // Get $getItemsCount products status(s) from walmart
            $productArray = $wishConfig->getItems(['limit' => $getItemsCount,'offset' => $offset,'publishedStatus'=>'']);
            ...
            elseif (isset($productArray['MPItemView']))
            {
                foreach ($productArray['MPItemView'] as $key => $value) {
                    ...
                    if (is_array($product) && count($product) > 0) {
                        //update main product status(s)
                        $query = "update wish_product set status='" . $value['publishedStatus'] . "' where product_id='" . $product['bigproduct_id'] . "' and merchant_id='".MERCHANT_ID."'";
                        Data::sqlRecords($query, null, 'update');
                    ...
                    if (is_array($productVariant) && count($productVariant) > 0) {
                        //update main product status(s)
                        $query = "update wish_product_variants set status='" . $value['publishedStatus'] . "' where option_id='" . $productVariant['option_id'] . "' and merchant_id='".MERCHANT_ID."'";
                        Data::sqlRecords($query, null, 'update');
    ...
    public function actionGettaxcode()
    {
        ...
        $html='';
        $id=trim(Yii::$app->request->post('id'));
        $query="select tax_code,cat_desc from wish_tax_codes where 1";
        ...
        try{
            ...
            foreach($post['promotion']['orignal_price'] as $key=>$price){
                ...
                if(isset($post['promotion']['id']) && isset($post['promotion']['id'][$key])){
                    $query = "UPDATE `wish_promotional_price` SET `original_price`='{$price}',`special_price`='{$post['promotion']['special_price'][$key]}',`effective_date`='{$post['promotion']['effective_date'][$key]}',`expiration_date`='{$post['promotion']['expiration_date'][$key]}' WHERE id='{$post['promotion']['id'][$key]}' ";
                     Data::sqlRecords($query,"one","update");
                }
                else
                {
                    $query = "INSERT INTO `wish_promotional_price` (`product_id`,`sku`,`merchant_id`,`original_price`,`special_price`,`effective_date`,`expiration_date`) VALUES($productId,'{$post['sku']}','{$post['merchant_id']}','{$post['promotion']['orignal_price'][$key]}','{$post['promotion']['special_price'][$key]}','{$post['promotion']['effective_date'][$key]}','{$post['promotion']['expiration_date'][$key]}') ";
