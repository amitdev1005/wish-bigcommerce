...
use frontend\modules\wishmarketplace\components\WishProduct;

class ProductcsvController extends WishmainController
{
    protected $wishHelper;
    ...
    public function actionExport()
    {
        ...
        foreach ($model as $value) {
            ...
            //print_r($value->bigproduct_id);die("dfgd");
            $product_price = Data::sqlRecords("SELECT `product_price`,`product_title` FROM `wish_product` WHERE `merchant_id`='" . $merchant_id . "' AND `product_id`='" . $value->bigproduct_id . "'", 'one');
            // print_r($product_price);die("dfgdf");
            if ($value->type == "simple") {
                ...
                if (is_array($optionResult) && count($optionResult) > 0) {
                    foreach ($optionResult as $key => $val) {
                        ...
                        $product_info = Data::sqlRecords("SELECT `option_prices` FROM `wish_product_variants` WHERE `option_id` = '" . $val['option_id'] . "' AND `merchant_id`='" . $merchant_id . "'", 'one');
    ...
    public function actionReadcsv()
    {
        ...
        if (isset($_FILES['csvfile']['name'])) {
            ...
            if (($handle = fopen($target, "r"))) {
                ...
                $status = WishProductModel::PRODUCT_STATUS_UPLOADED;
                //$stage = WishProductModel::PRODUCT_STATUS_STAGE;
                $unpublished = WishProductModel::PRODUCT_STATUS_UNPUBLISHED;
                $allpublishedSku = WishProduct::getAllProductSku($merchant_id, $status);

                  
                if($action=='qty' || $action=='price'){

                    //$allstageSku = WishProduct::getAllProductSku($merchant_id, $stage);
                    $allunpublishedSku = WishProduct::getAllProductSku($merchant_id, $unpublished);
                    $allpublishedSku = array_merge($allpublishedSku,$allunpublishedSku);
                ...
                while (($data = fgetcsv($handle, 90000, ",")) !== FALSE) {
                    ...
                    if ($action != 'upc') {
                        ...
                        //print_r($pro_sku);die("dhfjg");
                        if (in_array($pro_sku, $allpublishedSku)) {
                            $import_errors[$row] = 'Row ' . $row . ' : ' . 'Sku => "' . $pro_sku . '" is invalid/not published on wish.';
                            continue;
                        }
                    }
                ...
                if (count($selectedProducts)) {
                    ...
                    if ($wishConfig) {
                        $this->wishHelper = new WishProduct($wishConfig['client_id'], $wishConfig['client_secret_key'],$wishConfig['access_token']);
                        ...
                        Yii::$app->session->setFlash('warning', "Please enter wishapi...");
                    }
                    ...
                    if (count($import_errors)) {
                        ...
                        Yii::$app->session->setFlash('error', "None of your product(s) are published in Wish from csv....");
                    }
                }
            } else {
    ...
    public function actionPriceupdate()
    {
        ...
        $wishConfig = Data::sqlRecords("SELECT `client_id`,`client_secret_key`,`code`,`access_token` FROM `wish_configuration` WHERE merchant_id='".MERCHANT_ID."'", 'one');
        if ($wishConfig) {
            $this->wishHelper = new WishProduct($wishConfig['client_id'], $wishConfig['client_secret_key'],$wishConfig['code'],$wishConfig['access_token']);
      
            $response = $this->wishHelper->updatePriceviaCsv($products['products']);
            ...
            if (isset($response['errors'])) {
                ...
                $returnarr['count'] = count($response['success']);
            } else {
    ...
    public function actionInventoryupdate()
    {
        ...
        if ($wishConfig) {
            ...
            $response = $this->wishHelper->updateInventoryViaCsv($products['products']);
           // print_r($response);die;
            if (isset($response['errors'])) {
                ...
                $returnarr['success'] = "Product Information has been updated successfully";
                $returnarr['count'] = count($response['success']);
                // print_r($returnarr['count']);die("sfs");
            } else {
                $returnarr['error'] = "Products is not updated.";
            }
