...
use yii\helpers\Html;
	...
	$ipmort_url=\yii\helpers\Url::toRoute(['wishproduct/index']);
	$url=\yii\helpers\Url::toRoute(['wishproduct/batch-update-inventory']);
	$productUrl= \yii\helpers\Url::toRoute(['wishproduct/index']);
?>
        ...
        <div class="panel panel-default form new-section">
            <div class="jet-pages-heading">
                    <div class="title-need-help">
                        <h1 class="Jet_Products_style">Bulk Product Inventory Update</h1>
                    </div>
        	...
        	<div class="block-content panel-body shopify-api ">         
                <ul class="warning-div" style="margin-top: 18px">
					<li class="status_finish">
						...
						Starting Product price update execution, please wait...
					</li>
					<li class="status_warning">
						...
						Warning: Please do not close the window during price update process
					</li>
				</ul>
				...
				<ul id="profileRows">
					...
					<li id="liFinished" class="status_finish" style="display:none;">
						<img src="<?php echo yii::$app->request->baseUrl.'/frontend/images/batchupload/note_msg_icon.gif';?>" class="v-middle" style="margin-right:5px"/>
						Finished product price update execution.
					</li>
				</ul>
            </div>
        </div>     
		...
		j$.ajax({
        	...
        	success: function(json) {//transport
				...
				if(json.success) {
					...
					span.innerHTML = '<img class="v-middle" src="<?php echo $succes_img ?>"><span class="text">'+json.count+' products inventory of page '+id+'...</span>';
					span.id = 'id-'+id;
				...
				if(json.error){
							var span = document.createElement('li');
							span.innerHTML = '<img class="v-middle" src="<?php echo $error_img ?>"><span class="text">'+'Error Info : '+json.error.info+'</span>';
							//span.id = 'id-'+id;
							span.style = 'background-color:#FDD';
							update_row.parentNode.insertBefore(span, update_row); 	
				}
				if(id < pages) {
					...
					if(notSku && notSku>0){
						...
						span.innerHTML = '<img class="v-middle" src="<?php echo $succes_img ?>"><span id="update_status" class="text">'+countOfSuccess+' Product Successfully Uploaded.'+'</span>';
					}
					span.style = 'background-color:#DDF';
