			...
			<div class="image_popup">
				<div class="fieldset enable_api">
					<table class="table table-striped table-bordered">
						...
						{
							$obj = new Bigcomwal(Yii::$app->controller->id,'');
							$product=$obj->actionAddcustomproduct($value['bigproduct_id']);
							...
							//$additional=[];
	                  		...
	                  		$html.='<tr><td>';	                  		
                  			if(count(explode(',',$value['image']))>1)
                  			{
                  				$images=[];
                  				...
                  				$html.= Html::img($value['image'],
                  						['width' => '80px','height'=>'80px']);
                  			}
	    			        $html.='<br>'.$value['sku'].'</td>
	    			        <td>
	    			        	<form  action="#" method="post" class="form-image" id="form-data-'.$value['bigproduct_id'].'">
									<div class="form-group" id="image_contrainer_'.$value['bigproduct_id'].'">';
									for($i=0;$i<=count($product);$i++){
										foreach ($product as $k => $image){

											if($i==$image['sort_order']){
												$html.='<input type="checkbox" id="cbox2" name="image[]" value="'.$image['url_zoom'].'" ';
												foreach ($img as $m){
													if($m==$image['url_zoom']){ 
	                  									$html.='checked';
													}
												}
                  								$html.='> <label for="cbox2">';
												$html.= Html::img($image['url_zoom'],['width' => '120px','height'=>'120px']);
                  						        $html.='</label><br>';
											}
										}
									}
									$html.='
										...
										<input type="button" class="btn btn-primary" onclick=updateImage(this,"'.$value['bigproduct_id'].'") value="save"/>
										<img scr="'.Yii::$app->getUrlManager()->getBaseUrl().'/images/712.gif" style="display:none">
										<input type="hidden" name="id" value="'.$value['bigproduct_id'].'">
									</div>
								</form>
							</td></tr>';
						}
