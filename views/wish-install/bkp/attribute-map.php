...
foreach ($bigcom_product_types as $type_arr) 
{
    ...
    // $category = explode(" ",$WishCategoryId);
    // print_r($category);die();
    $parent_id = explode(",",$type_arr['category_path']);
    $wishAttributes = [];
    if(!is_null($WishCategoryId)) {
        $wishAttributes = AttributeMap::getWishCategoryAttributes($WishCategoryId,$parent_id[0])?:[];
    }
    // print_r($wishAttributes);die();
											...
											?>
												<tr>
													...
													<?php
														...
														// print_r($attribute); echo "baby";F[]
														if(isset($attribute['wish_attributes']['attribute_values'][$wal_attr])) {
															$selectTypeFlag = true;
															$walmart_attribute = $attribute['wish_attributes']['attributes'][$wal_attr];
														}
													...
													?>
															<!-- <select name="walmart[<?= $attribute['product_type']?>][<?= $wal_attr ?>]">
																<option value=""></option> -->
													<?php   //print_r($walmart_attribute_values);die;
															//$walAttrValue = explode(',', $walmart_attribute_values);
										
															// foreach($walmart_attribute_values as $value) {
																// print_r($value);die("gh");
													?>
																<!-- <option value="<?= $walmart_attribute ?>"  -->
																<?php if(in_array($walmart_attribute,explode(',', $mapped_value)) && $valueType==WishAttributeMap::VALUE_TYPE_WISH)
																	  { echo 'selected="selected"'; } ?>
																<?= $walmart_attribute ?><!-- </option> -->
