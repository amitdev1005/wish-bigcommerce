...
$this->title = 'BigCommerce-Jet Category Mapping';
	...
	<form id="category_map" method="post" action="<?php echo \yii\helpers\Url::toRoute(['categorymap/save']) ?>">
		...
		<table id="map_producttype" class="table table-striped table-bordered">
			<tr>
				...
				<th colspan="3">Jet Category Name</th>
			</tr>
			<?php 
			$i=0;
			foreach($model as $value){
				...
				<tr>
					...
					<td><?php echo $value->product_type; ?></td>
		  			<td class="cat_root">
						<select type="<?php echo $value->product_type ?>?>" style="width:auto" class="form-control" onchange="selectChild(this,1)">
					...
					<td class="cat_child"></td>
					<td class="cat_sub_child"></td>	
				</tr>
				<?php 
			}
			?>
...
function selectChild(node,level){
	console.log(j$(node).val());
	var url='<?php echo $url ?>';
	j$.ajax({
        method: "GET",
        url: url,
        data: { id: j$(node).val(),level: level,_csrf : csrfToken }
      })
      .done(function( msg ) {
          console.log(msg);
         //j$('#jet_Attrubute_html').html('');
         //j$('#jet_Attrubute_html').html(msg);
      });
}
</script>