    ...
    <form id="attribute_map" class="form new-section" method="post" action="<?= Data::getUrl('wish-attributemap/save') ?>">
		...
		<div class="grid-view table-responsive">
			...
			<?php
							...
							if(isset($attribute['wish_attributes']['bigcom_attributes'][$wal_attr])) {
								$selectTypeFlag = true;
								$bigcom_attribute[] = $attribute['bigcom_attributes'][$wal_attr];
							}
							if(!$selectTypeFlag && strpos($val, AttributeMap::ATTRIBUTE_PATH_SEPERATOR) !== false)
							{
								$wal_attrs = explode(AttributeMap::ATTRIBUTE_PATH_SEPERATOR, $val);
								if(is_array($wal_attrs)) {
									foreach ($wal_attrs as $_wal_attr) {
										if(isset($attribute['wish_attributes']['attribute_values'][$_wal_attr])) {
											$selectTypeFlag = true;
											$bigcom_attribute[] = $attribute['wish_attributes']['attribute_values'][$_wal_attr];
										}
									}
								}
							}
			...
			?>
								<select name="walmart[<?= $attribute['product_type']?>][<?= $wal_attr ?>]">
									<option value=""></option>
			<?php    
								foreach ($bigom_attribute as $value) {
			?>
									<option value="<?= $value ?>" 
