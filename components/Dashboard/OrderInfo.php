...
class OrderInfo extends Component
{
    ...
    public static function getCompletedOrdersCount($merchantId)
    {
        if(is_numeric($merchantId)) {
            $result = [];
            $total = 0;
            $query = "SELECT COUNT(*) FROM `wish_order_details` WHERE `status` = 'shipped' AND `merchant_id` ={$merchantId}";
    ...
    public static function getAcknowledgedOrdersCount($merchantId)
    {
       if(is_numeric($merchantId)) {
            $result = [];
            $total = 0;
            $query = "SELECT COUNT(*) FROM `wish_order_details` WHERE `status` = 'acknowledged' AND `merchant_id` ={$merchantId}";
            $result = Data::sqlRecords($query, 'all');
            $total = is_array($result)?$result[0]["COUNT(*)"]:0;
            return $total;
        }        
    ...
    public static function getCancelledOrdersCount($merchantId)
    {
         if(is_numeric($merchantId)) {
            $result = [];
            $total = 0;
            $query = "SELECT COUNT(*) FROM `wish_order_details` WHERE `status` = 'canceled' AND `merchant_id` ={$merchantId}";
            $result = Data::sqlRecords($query, 'all');
            $total = is_array($result)?$result[0]["COUNT(*)"]:0;
            return $total;
        }     
    ...
    public static function getTotalOrdersCount($merchantId)
    {
        if(is_numeric($merchantId)) {
            $result = [];
            $total = 0;
            $query = "SELECT COUNT(*) FROM `wish_order_details` WHERE `merchant_id` ={$merchantId}";
            $result = Data::sqlRecords($query, 'all');
            $total = is_array($result)?$result[0]["COUNT(*)"]:0;
            return $total;
        }      
    ...
    public function removeFailedOrders($merchant_id)
    {
        ...
        if(count($failedOrdersList)>0){
            $orderExist = array();
            $orderExist = Data::sqlRecords("SELECT `merchant_order_id` FROM `wish_order_details` WHERE `merchant_id`='".$merchant_id."' AND `merchant_order_id` IN('".implode("' , '", $failedOrdersList)."') ","all","select");
