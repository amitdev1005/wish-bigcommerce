...
class Earninginfo extends Component
{
    ...
    public static function getTodayEarning($merchant_id)
    {
        ...
        if(is_numeric($merchant_id)) {
            ...
            $query = "SELECT `order_total` FROM `wish_order_details` WHERE `created_at` BETWEEN '{$date1}' AND '{$date}' AND `status` = 'shipped' AND `merchant_id` ={$merchant_id}";
    ...
    public static function getMonthlyEarning($merchant_id)
    {
        ...
        if(is_numeric($merchant_id)) {
            $query = "SELECT `order_total` FROM `walmart_order_details` WHERE `created_at` BETWEEN '{$date1}' AND '{$date}' AND `status` = 'shipped' AND `merchant_id` ={$merchant_id}";
            $result = Data::sqlRecords($query, 'all');
            $total = 0;
            if(is_array($result) && count($result)>0)
            {
    ...
    {
       if(is_numeric($merchant_id)) {
            $query = "SELECT `order_total` FROM `walmart_order_details` WHERE `status` = 'shipped' AND `merchant_id`=".$merchant_id;
            $result = Data::sqlRecords($query, 'all');
