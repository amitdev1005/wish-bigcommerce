...
class WishProduct extends Wishapi
{
   
    const GET_ITEMS_UPDATE_SUB_URL = "v2/variant/update";
    const ALL_PRODUCT_UPLOAD_FILEPATH = '/frontend/modules/wishmarketplace/filestorage/product/create/';
    ...
    public  function updatePriceviaCsv($products = [])
    {
        ...
        $priceArray = stream_context_create(array(
          'http' => array(
            'method' => 'POST',
            'ignore_errors' => true,
            ),
        ...
        if (is_array($products) && count($products) > 0) {
            foreach ($products as $product) {
                 ...
                 $priceArray = [
                            ...
                            // $priceArray=$product;
                            if($priceArray){
                            $response=$this->CPutRequest(self::GET_ITEMS_SUB_URL,$priceArray);
        ...
        if ($isPriceFeed > 1) {
            ...
            if (!$merchant_id)
                $path = 'wishmarketplace/product/update/' . date('d-m-Y') . '/' . MERCHANT_ID . '/csv/inventory';
            else
                $path = 'wishmarketplace/product/update/' . date('d-m-Y') . '/' . $merchant_id . '/csv/inventory';
    ...
    */  
        ...
        if($response['success']==1 || $response['success']== ""){
                ...
                foreach ($products as $productsvalue) {
                    ...
                    if ($productsvalue['type'] == "No Variants") {
                        $query = "update `wish_product` set product_price ='" . $productsvalue['price'] . "' where merchant_id='" . $merchant_id . "' AND product_id='" . $productsvalue['option_id'] . "'";
                    // print_r($query);
                        ...
                        $query = "update `wish_product_variants` set option_prices ='" . $productsvalue['price'] . "' where merchant_id='" . $merchant_id . "' AND option_id='" . $productsvalue['option_id'] . "'";
                        Data::sqlRecords($query, null, "update");
                    }
                    $count++;
    ...
    public  function updateInventoryViaCsv($products = [])
    {
        ...
        if ($count > 1) {
            ...
            // print_r($response['success']);die("dgcdfcd");  
            ...
            if ($response['success'] == 1 || $response["success"]=="") {
                foreach ($products as $productsvalue) {
                    ...
                    if ($productsvalue['type'] == "No Variants") {
                        ...
                        $query = "UPDATE `jet_product` SET `qty`='" . $productsvalue['qty'] . "' WHERE `merchant_id`='" . $merchant_id . "' AND `id`='" . $productsvalue['id'] . "'";
                        Data::sqlRecords($query, null, "update");
                       // print_r($query);die("dsfds");
                        Data::createLog("inventory jet_product query: " . PHP_EOL . $query . PHP_EOL, $logFile);

                    } else {
                        if($productsvalue['type']== 'Parent'){
                            $query = "update `wish_product` set product_price ='" . $productsvalue['price'] . "' where merchant_id='" . $merchant_id . "' AND id='" . $productsvalue['id'] . "'";
                            Data::sqlRecords($query, null, "update");
                        }
    ...
    public static function getVariantsProductInfo($product_ids)
    {
        ...
        $query = "SELECT " . implode(',', $jet_columns) . "," . implode(',', $walmart_columns) . " FROM `jet_product_variants` `jpv` INNER JOIN (SELECT * FROM `walmart_product_variants` WHERE `merchant_id`='{$merchant_id}' AND `product_id` IN ({$product_ids})) AS `wpv` ON `jpv`.`option_id`=`wpv`.`option_id`";
        $result = Data::sqlRecords($query, 'all', 'select');
    ...
    public static function uploadAllProductsOnWalmart($ids, $merchant_id)
    {
        if (count($ids) > 0) {
