...
public function CPostRequest($method,$postFields){
    if($method== self::ITEMS_DELETE_SUB_URL)
    {
      $url = 'https://merchant.wish.com/api/v2/product/disable';
    }
    else if($method== self::GET_FEEDS_ITEMS_ADD_SUB_URL)
    {
...
public function CGetRequest($method,$postFields){

  
    if($method == self::GET_ITEMS_SUB_URL)
    {
      $url ='https://merchant.wish.com/api/v2/product';
    }
    else if($method == self::GET_ORDERS_SUB_URL)
    {
     
      $url ='https://merchant.wish.com/api/v2/order';
    }
...
public function CGetFeed($method,$postFields){
  ...
  if (is_array($response)) {
      if($response['code']==0){
        $data = $response['data']; 
        if(isset($data['Product']) && $data['Product'])
          return array('success'=>true ,'status_on_wish' =>$data['Product']);
      else if(isset($data['Variant']) && $data['Variant'])
          return array('success'=>true ,'status_on_wish' => $data['Variant']);
      else if(isset($data['Order']) && $data['Order'])
          return array('success'=>true ,'order' => $data);
      else if(isset($data['job_id']) && $data['job_id'])
          return array('success'=>true ,'job_id' => $data);
  ...
  return array('success'=>false ,'message'=> $result);
    ...
    public function getOrders($order_id)
    {
      $productUpload = stream_context_create(array(
        ...
        if($productUpload){
          $response=$this->CGetRequest(self::GET_ORDERS_SUB_URL,$productUpload);
...
public function replaceNs($response)
{
  ...
  return $response;
     ...
     */
    public function getFeeds($feedId = null, $subUrl = self::GET_ITEMS_SUB_URL)
    {
      $response = "";
      $feed['parent_sku'] =$feedId;
      if ($feedId != null) {
        $response=$this->CGetFeed($subUrl,$feed);
        print_r($response);die;
        return $response;
    }
    return $response;
  ...
  public function retireProduct($sku = null)
  {
    ...
    $productDelete = stream_context_create(array(
      ...
      if($response['success']==1){
          $response['message'] = "Price Disabled on wish";
        ...
        foreach($ids as $id) 
        {
          ...
          if ($productArray) 
          {
            ...
            if (is_array($validateResponse) && isset($validateResponse['error'])) {
              ...
              // print_r($uploadErrors);die("fg88hfg");
              continue;
          } 
...
*/
              ...
              //print_r($productArray);die("dfgdfg");
                $product = [
                ...
                'shipping' => "0.0",
                'inventory' =>$value['option_qty'],// 'tax_code' => $tax_code,
                'brand' => $brand,
                'main_image' => $productArray['image'],
