...
class WishPromoStatus extends component
{
	...
	public static function getPromoStatus($productIds)
	{
		try{
			...
			$query = "";
	        $query = $connection->createCommand("SELECT `sku`,`option_id`,`product_id` FROM `wish_promotional_price` WHERE `product_id` IN(".$productIds.") GROUP BY `sku`");
	        $promos = $query->queryAll();
	        $url = "v3/promo/sku/";
	        $wal = new Wishapi(API_USER, API_PASSWORD, CONSUMER_CHANNEL_TYPE_ID);
	        if(count($promos)==0){
	        ...
	        foreach ($promos as $promo) {
	        	...
	        	$count = 0;
	        	$error = 0;
	        	if(Wishapi::is_json($response)){
		                $responseArray = json_decode($response,true);
		        }else{
		            $response = $wal->replaceNs($response);
		            $responseArray = Wishapi::xmlToArray($response);
		        }
		            	...
		            	else
		            	{
		            		...
		            		if(isset($pricingList['_attribute']))
		            		{
			            		...
			            		else
			            		{
				            		...
				            		$query = "UPDATE `wish_promotional_price` SET `walmart_promo_id`= '".trim($attributes['promoId'])."', `walmart_status` = '".trim($status)."' WHERE `id`='{$exist['id']}'";
			            		}
	        ...
	        if(count($uploaded_ids)) {
	        	$query = "UPDATE `wish_promotional_price` SET `walmart_promo_id`= '', `walmart_status` = '".self::PROMOTIONAL_PRICE_STATUS_PENDING."', `to_delete`=0 WHERE `merchant_id`=".MERCHANT_ID." AND `id` NOT IN (".implode(',', $uploaded_ids).")";
	...
	{
		$query = "DELETE FROM `wish_promotional_price` WHERE `sku`='{$productSku}' AND `merchant_id`='{$merchant_id}' AND `to_delete`=1 AND `walmart_status`='{self::PROMOTIONAL_PRICE_STATUS_PROCESSING}'";
	...
	{
		$query = "UPDATE `wish_promotional_price` SET `walmart_status`='{self::PROMOTIONAL_PRICE_STATUS_PENDING}' WHERE `sku`='{$productSku}' AND `merchant_id`='{$merchant_id}' AND `walmart_status`='{self::PROMOTIONAL_PRICE_STATUS_PROCESSING}'";
	...
	{
		$query = "SELECT COUNT(*) as `promo` FROM `wish_promotional_price` WHERE `sku` LIKE '{$productSku}' AND `merchant_id`='{$merchant_id}'";
