...
public function CGetRequest($method,$postFields){
    ...
    // https://sandbox.merchant.wish.com/api/v2/product 
    $curl = curl_init();
    $options = array(
      CURLOPT_CONNECTTIMEOUT => 10,
      CURLOPT_RETURNTRANSFER => true,
        ...
        foreach($ids as $id) 
        {
          ...
          if ($productArray) 
          {
            ...
            $validateResponse = self::validateProduct($productArray);
                   // print_r($validateResponse);die("dfgdf");
...
*/
      ...
      elseif(isset($responseId)){
          ...
          return['success'=>$responseId,"Product Uploaded on Wish Store"];
      }  
  ...
  if (isset($responseArray['FeedAcknowledgement']['feedId'])) {
    ...
    public function updateWishprice($product = [])
    {
        ...
        if($productUpload){
          ...
          if($response['success']==1){
            return ['success'=>'Price Updated on Wish'];
          }
          else {
