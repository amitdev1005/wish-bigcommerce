...
class WishRepricing extends Wishapi
{
	 ...
	 * @return array 
	 */
	public static function getBestMarketplacePrice($walmartItemId,$upc=false)
	{
		...
		{
			return $result = Wishapi::xmlToArray($output);
		}
		else
		{
	...
	public static function getSavedRepricingData($product_id)
	{
		try
		{
			$query = 'SELECT * FROM `wish_product_repricing` WHERE product_id="'.$product_id.'"';
	...
	public static function getProductData($product_id)
	{
		try
		{
			...
			$query = 'SELECT product_id,title,sku,type,wal.product_type,wal.status,description,image,qty,price,weight,upc,walmart_attributes,category,tax_code,short_description,self_description,common_attributes,sku_override,product_id_override FROM (SELECT * FROM `wish_product` WHERE `product_id` ="'.$product_id.'") as wal INNER JOIN (SELECT * FROM `jet_product` WHERE `bigproduct_id` ="'.$product_id.'") as jet ON jet.bigproduct_id=wal.product_id WHERE wal.product_id="'.$product_id.'" LIMIT 1';
	...
	public static function getProductIdsBySku($sku, $merchant_id)
	{
		try
		{
			$main_table = "SELECT `product_id`, `variant_id` FROM `wish_product` `wp` INNER JOIN (SELECT * FROM `jet_product` WHERE `sku`='{$sku}' AND `merchant_id`={$merchant_id}) as `jp` ON `jp`.`bigproduct_id`=`wp`.`product_id`";


			$variant_table = "SELECT `wpv`.`product_id`, `wpv`.`option_id` as `variant_id` FROM `wish_product_variants` `wpv` INNER JOIN (SELECT * FROM `jet_product_variants` WHERE `option_sku`='{$sku}' AND `merchant_id`={$merchant_id}) as `jpv` ON `wpv`.`option_id`=`jpv`.`option_id`";
	...
	{
		$query = "SELECT `id` FROM `wish_product_repricing` WHERE `merchant_id` = {$merchant_id} AND `sku` LIKE '{$sku}' LIMIT 0,1";
		$result = Data::sqlRecords($query, "one", "select");
	...
	public function saveProductVariant($product_id, $option_id, $data=[], $table='jet_product_variants')
	{
		$merchant_id = MERCHANT_ID;
		try {
	 ...
	 * @return void 
	 */
	public function fetchWalmartProductReport()
	{
		$url = Wishapi::GET_REPORTS_SUB_URL."?type=item";
	...
	{
		$url = Wishapi::GET_REPORTS_SUB_URL."?type=buybox";
	...
	public static function getProductPrice($orignal_price,$type,$id,$merchant_id)
	{
		...
		if($type == 'simple')
		{
			...
			$query = "SELECT * FROM `wish_product_repricing` WHERE `product_id` = '{$id}' AND `repricing_status` = '1' LIMIT 0,1";
			$result = Data::sqlRecords($query,"one","select");
			if($result) {
		...
		elseif($type == 'variants')
		{
			...
			$query = "SELECT * FROM `wish_product_repricing` WHERE `option_id` = '{$id}' LIMIT 0,1";
			$result = Data::sqlRecords($query,"one","select");
			if($result) {
				$minPrice = floatval($result['min_price']);
				$maxPrice = floatval($result['max_price']);
	 ...
	 * @return int
	 */
	public static function getBestMktPrice($repriceData)
	{
	 ...
	 * @return int
	 */
	public static function calculateBestPrice($orignal_price,$bestMktPrice,$minPrice,$maxPrice)
	{
		$price = 0;
	 ...
	 * @return string|bool
	 */
	public static function downloadBuyBoxReport($merchant_id, $filePath=null)
	{
		...
		$query = "SELECT * FROM `wish_buybox_report` WHERE NOW() > ADDTIME(`download_time`, '".$interval_time."') AND `merchant_id`={$merchant_id} LIMIT 0,1";
		$result = Data::sqlRecords($query,'one');

		$isExist = Data::sqlRecords("SELECT * FROM `wish_buybox_report` WHERE `merchant_id`={$merchant_id} LIMIT 0,1",'one');
		...
		if($result || !$isExist)
		{
			...
			if($this->fetchWalmartBuyboxReport($filePath)) 
			{
				...
				if($isExist)
				{
					$query = "UPDATE `wish_buybox_report` SET `timestamp` = '{$time}' WHERE `id` = {$result['id']}";
					Data::sqlRecords($query,null,'update');
				}
				else
				{
					$query = "INSERT INTO `wish_buybox_report` (`merchant_id`, `timestamp`) VALUES ({$merchant_id},'{$time}')";
	 ...
	 * @return string|bool
	 */
	public static function extractZipFile($filePath, $extract=true)
	{
		try
		{
			$zip = new ZipArchive();
	 ...
	 * @return array
	 */
	public static function readBuyboxCsv($csvFilePath, $limit=null, $page=0)
	{
		$buyBoxReport = [];
	...
	public static function isRepricingEnabled($attribute,$merchant_id=null)
	{
		...
		if($where != '') 
		{
			$query = 'SELECT `id` FROM `wish_product_repricing` WHERE '.$where.' AND `repricing_status`=1 AND `merchant_id`="'.$merchant_id.'" LIMIT 0,1';
			$result = Data::sqlRecords($query, 'one');
			if(!$result) {
				return false;
			} else {
