...
class Wishapi extends Component
{
    ...
    const GET_FEEDS_ITEMS_SUB_URL = 'v2/product/add';
...
*/
   /* public $apiUrl;
    ...
    {
        $this->apiUrl = "https://marketplace.wishapis.com/";
 ...
 public function CPostRequest($method,$postFields){
        ...
        $url= $this->client_id.$method.http_build_query($postFields);
        ...
        $headers[] = "Authorization: Bearer ".$tObject;
            ...
            foreach ($ids as $id) 
            {
                ...
                if ($productArray) 
                {
                    ...
                    elseif($validateResponse === true)
                    {
                        ...
                        $image1=trim($productArray['additional_images']);
                        $imageArr = explode(',', $image);
                        $imageArr1=explode(',',$image1);

                        if($image1){
                            foreach($imageArr1 as $value){
                       ...
                       // $tax_code = $tax_code = Data::GetTaxCode($productArray, $merchant_id);
...
*/
            ...
            //print_r($context);die("dff");
            /*print_r($productToUpload);
            WishProductComponent::generateXml($productToUpload);
            die('end');*/
           ...
           // print_r($key);die;
            if ($key == 1) {
                ...
                $xml = new Generator();
                $xml->arrayToXml($context)->save($file);
                ...
                }elseif(is_null($responseArray))
                {
                    return ['errors' => 'An error occurred while processing your request.'];
                }
            }
   ...
   /* public function prepareMPItem($merchant_id, $product, $productArray, $variantArray=null)
    {
        ...
        $mpItem = WishProductComponent::getMPItemStructure();

        if ($product['product_status'] == WishProduct::PRODUCT_STATUS_NOT_UPLOADED) {
            $processMode = 'CREATE';
        } else {
        ...
        $mpItem['productIdentifiers'] = WishProductComponent::getProductIdentifiersValue([['identifier_type'=>$product['identifier_type'], 'identifier_value'=>$product['upc']]]);
        ...
        $validate = WishProductComponent::validateStructure($mpItem);
        if(!$validate['status']) {
            $error = isset($validate['error']) ? $validate['error'] : 'Required MpItem Attributes not filled.';
            return ['status' => false, 'error' => $error];
        }
                ...
                foreach ($product as $pro) {
                    ...
                    if ($pro['type'] == 'variants') {
                            ...
                            if (isset($pro['prices']) && !empty($pro['prices'])) {
                                $pro['price'] = WishRepricing::getProductPrice($pro['prices'], 'variants', $pro['option_id'], MERCHANT_ID);
                            }
                            else{
                                $pro['price'] = WishRepricing::getProductPrice($pro['price'], 'variants', $pro['option_id'], MERCHANT_ID);
                            }
                        ...
                        if (isset($price['product_price']) && !empty($price)) {
                            $pro['price'] = WishRepricing::getProductPrice($price['product_price'], 'simple', $pro['id'], MERCHANT_ID);

                        }
                        else{
                            $pro['price'] = WishRepricing::getProductPrice($pro['price'], 'simple', $pro['id'], MERCHANT_ID);
                        }
                        $key += 1;
                        $priceArray['PriceFeed']['_value'][$key] = [
                            'Price' => [
        ...
        if ($isPriceFeed > 0) {
          ...
          /*   if($isEnablePutRequest){
                ...
                if (isset($responseArray['FeedAcknowledgement'])) {
                    ...
                    return ['success' => true, 'message' => 'Price Feeds is successfully submitted on wish'];
   ...
   public function updateWalmartinventory($product = [])
    {
        ...
        if (count($product) > 0 && $isInvFeed > 0) {
            ...
            if($isEnablePutRequest){
                ...
                print_r($response);die("updateinventory");
                $responseArray = self::xmlToArray($response,true);
                if (isset($responseArray['inventory']['sku'])) {
                        return ['success' => true, 'message' => 'Inventory Feeds is successfully submitted on wish'];
                    }
               elseif (isset($responseArray['errors'])) {
                    return ['errors' => $responseArray['errors']];
                }
