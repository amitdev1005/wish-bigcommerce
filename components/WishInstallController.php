...
class WishInstallController extends WishmainController
{
	...
	public function actionSaveCategoryMap()
	{
    	...
    	// print_r($data);die("wishinstallcontroller");
    	if($data && isset($data['type']))
    	{
    		...
    		foreach($data['type'] as $key=>$value)
    		{
    			...
    			if(is_array($value) && count($value)>0 && $value[0]!="")
    			{
                    ...
                    // $step=WishCategory::getCategoru($value['id']);
		            ...
		            $query1 = 'UPDATE `wish_category_map` SET  category_id="'.trim($category_id).'",category_path="'.trim($category_path).'" where merchant_id="'.$merchant_id.'" and product_type="'.$key.'"';
		            $model = Data::sqlRecords($query1, null, 'update');
		            
		            $query2 = 'UPDATE `wish_product` SET  category="'.trim($category_id).'" where merchant_id="'.$merchant_id.'" and product_type="'.$key.'"';
		            $product = Data::sqlRecords($query2, null, 'update');
    			}
    			else
    			{
                    // $taxcode = isset($value['taxcode'])?$value['taxcode']:'';
                    $query1 = 'UPDATE `wish_category_map` SET  category_id="",category_path="" where merchant_id="'.$merchant_id.'" and product_type="'.$key.'"';
                    $model = Data::sqlRecords($query1, null, 'update');
