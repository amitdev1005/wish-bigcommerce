...
class Wishapi extends Component
{
    ...
    const GET_FEEDS_ITEMS_ADD_SUB_URL = "/product/add";
    ...
    //By Shivam
    public function retireProduct($sku = null, $subUrl = self::GET_ITEMS_UPDATE_SUB_URL)
    {
        $response = "";
        if ($sku != null) {
    ...
    public function getItemstatus($sku, $returnField = null, $subUrl = self::GET_ITEMS_UPDATE_SUB_URL)
    {
                    ...
                    elseif($validateResponse === true)
                    {
                       ...
                       // $tax_code = $tax_code = Data::GetTaxCode($productArray, $merchant_id);
                        ...
                        if ($productArray['type'] == "simple") 
                        {
                            ...
                            $product = [
                                //'access_token'=> 'c8fbc810df0d4afa9bb0063c5ed67f57',
                                'id'=>$productArray['product_id'],
                                'name' => $title,
                                //'parent_sku' => $productArray['sku'],
...
*/
                ...
                if($productUpload){
                  //print_r($productArray['status']);die("dgffg");
                    if($productArray['status'] == "Not Uploaded"){

                        $response1=$this->CPutRequest(self::GET_ITEMS_UPDATE_SUB_URL,$productUpload);
                    ...
                    //$query="UPDATE `status`=`Item Processing` where bigproduct_id= "
        ...
        $productUpload=$product;
            if($productUpload){
                ...
                if($response['success']==1){
                    $responseArray['success'] = ['Product Updated on Wish'];
                }
                else {
                    $responseError[] = $response['message'];
                    $responseArray['errors'] = "Product Upload failed due to".$responseError;
                }
            } 
        return $responseArray;
    }
        ...
        foreach($product as $pro)
        {
            ...
            if($productUpload){
                ...
                if($response['success']==1){
                    ...
                    $response['error'] = "Price Updation Failed";
                    $responseArray['error'] = $response['error'];
                }
            }
        }
        return $responseArray;
   ...
   public function updateWishinventory($product = [])
    ...
    public function getItem($sku, $returnField = null, $subUrl = self::GET_ITEMS_UPDATE_SUB_URL)
    {
        $response = $this->getRequest($subUrl . '?sku=' . $sku);
        try {
            $response = json_decode($response, true);
