...
class WishCategory extends Component
{
    ...
    public static function getrootcategory($category_name = null)
    {
        ...
        //         if(isset($value['0']))
        //         {
        //             if(in_array($value['0'],$checkindname))
        //             {
        //                 continue;
        //             }
        //             else
    ...
    public static function getcategorytree()
    {
        ...
        // Get all category Name From CSV File
        $checkindname=array();
        $extra_arr = array();
        foreach($category_tree_data as $key=>$value)
        {
            ...
            {
                $indname[$value['1']][$value['3']][$value['5']]=$extra_arr;
                $checkindname[]=$value['3'];
            }
        }
        ...
        foreach($category_tree_data as $key=>$value)
        {
            if(isset($value['3']))
            {
                if(in_array($value['3'],$checksubcate))
                {
                    continue;
                }
                else
                {
                    $subname[$value['3']] = $value['3'];
                    $checksubcate[]=$value['3'];
                }
            }
        }
        
        $return_arr[] = $category_indname;
        $return_arr[] = $subname;
        return $return_arr;
    }
    ...
    public static function getCategoryVariantAttributes($category)
    {
        ...
        if (!isset($session[$index])) {*/
            ...
            if ($category != '') {
                ...
                $attrs = Wishapi::isValidateVariant($parent_id);
                if(count($attrs)) {
...
//            print_r($variantAttributes);die;
    ...
    {
        $query = 'SELECT `title`,`parent_id`,`attributes`,`attribute_values`,`walmart_attributes`,`walmart_attribute_values` FROM `wish_category` WHERE `category_id`="' . $category_id . '" LIMIT 0,1';
     ...
     public static function getWalmartCategory($category)
    {
        ...
        if (!isset($session[$index])) {
            $query = "SELECT * FROM `wish_category` WHERE `category_id`='" . $category . "' LIMIT 0,1";
    ...
    {
        $index = 'wish_category_' . addslashes($category);
        return $index;
    }
    public static function getCategoryName($category_id)
    {
        ...
        $str = file_get_contents($maincategoryroute);
        $maincategory = array_map("str_getcsv", preg_split('/\r*\n+|\r+/', $str));
        $category_tree_middle ="";
        $length=count($maincategory);
        for($i=1;$i<=$length;$i=$i+1)
        {
            if($i==0 && $i<=$length)
            {
                $head_data=$maincategory[$i];
            }
            else
            {   if($i<=$length)
                {
                    if(isset($maincategory[$i]))
                    {
                        $category_tree_middle[]=$maincategory[$i];
                    }
                }
        ...
        $category_tree_data=$category_tree_middle;
        foreach($category_tree_data as $key=>$val)
        {
            if(isset($val['0']))
            {
