...
/*function manage_counter() {
			if($('div').has('.trial-nav-wrap')){
				var ht = $('.navbar-default').height();
	            $('.fixed-container-body-class').css('padding-top',ht+30 );
	        }
		}
*/
...
function manage_counter()
{
    if($(document).find('.notification').length){
        var th = $('.notification').height();
        $('.navbar-default').css('margin-top',th+20);
        if($('div').has('.fixed-container-body-class')){
            $('.fixed-container-body-class').css('padding-top',th+73 );
        }
    }
    else{
